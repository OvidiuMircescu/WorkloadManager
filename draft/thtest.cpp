...
private:
  ...
  void finishTasks();
private:
  std::map<int, std::thread> _tasks;
  std::queue<int> _finishedTasks;
  int _nextIndex = 0;
  std::mutex _data_mutex;
  std::condition_variable _condition;
  bool _stop = false;
  std::list< std::thread> _otherThreads;
};
...
void Manager::addTask()
{
  ...
  _tasks.emplace(currentIndex, [this, currentIndex]
    {
...
void Manager::start()
{
  _otherThreads.emplace_back([this]
    {
      finishTasks();
    });
}
...
void Manager::stop()
{
  ...
  for(std::thread& th : _otherThreads)
    th.join();
